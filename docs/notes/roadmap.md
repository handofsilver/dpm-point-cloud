...
|---|---|---|
| 安装 `geomloss` | pip | ✅ 完成 |
| `chamfer_distance`（CD） | `metrics.py` | ✅ 完成（原有实现保留） |
| `earth_mover_distance`（EMD，Sinkhorn 近似） | `metrics.py` | ✅ 完成 |
| `_pairwise_cd` / `_pairwise_emd`（成对距离矩阵） | `metrics.py` | ✅ 完成 |
| MMD / COV / 1-NNA（`_mmd_cov_1nna` + `compute_all_metrics`） | `metrics.py` | ✅ 完成 |
...
**`eval_ae.py` 职责**：遍历测试集 → encode → diffusion sample → 逐样本 CD/EMD → 打印 per-category 均值

#### 7-C & 7-D：评估脚本 + 跑出 Table 1 数字

| 任务 | 文件 | 状态 |
|---|---|---|
| 实现 `eval_gen.py`：大批量采样 → `compute_all_metrics` | `scripts/eval_gen.py` | ✅ 完成 |
| 在 chair/airplane/car 上跑出数字 | — | ⬜ 待训练 + 推理 |
...
| 阶段 | 核心工作 | 预估难度 | 状态 |
|---|---|---|---|
| 7-A | CD + EMD + 集合级指标实现 | ★★★ | ✅ 完成 |
| 7-B | `eval_ae.py` + 跑 Table 2 | ★★ | ✅ 完成（待训练后验证） |
| 7-C/D | `eval_gen.py` + 跑 Table 1 | ★★ | ✅ 完成（待训练后验证） |
| 剩余 | 跑训练 + 推理拿出实际数字 | compute-heavy | ⬜ |

> **下一步**：所有评估代码已就绪，剩余工作是跑训练（`train_ae.py` / `train_gen.py`）到收敛，再执行 `eval_ae.py` 和 `eval_gen.py` 拿出论文数字。
